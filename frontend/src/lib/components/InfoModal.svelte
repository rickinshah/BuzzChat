...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
	...
	>
		...
		<!-- Message -->
		<h2 class="mb-1 text-center text-xl font-semibold text-blue-400">Information</h2>
		{#if typeof infoMessage === 'string'}
			<p class="mb-4 text-center text-sm break-words text-gray-300">{infoMessage}</p>
		{:else}
			<ul class="mb-4 text-center text-sm break-words text-gray-300">
				{#each Object.entries(infoMessage) as [field, message]}
